    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-w-3xl">
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama_produk" id="nama_produk" required value="{{ old('nama_produk') }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#455546] focus:border-[#455546] sm:text-sm px-4 py-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="harga" id="harga" required value="{{ old('harga') }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#455546] focus:border-[#455546] sm:text-sm px-4 py-2">
                ...
                <div>
                    ...
                    <input type="number" name="stok" id="stok" required value="{{ old('stok') }}" class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#455546] focus:border-[#455546] sm:text-sm px-4 py-2">
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4" required class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#455546] focus:border-[#455546] sm:text-sm px-4 py-2">{{ old('deskripsi') }}</textarea>
                @error('deskripsi') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
            </div>

            <div>
                <label for="foto_produk" class="block text-sm font-bold text-gray-800 mb-2">Product Image (Optional)</label>
            ...
            <div class="pt-4 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#445344] hover:bg-[#364236]">Create Product</button>
            </div>
        </form>
    </div>
</x-admin-layout>
